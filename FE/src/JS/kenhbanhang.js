...
function togglePanel(panelId) {
    document.querySelectorAll('.panel').forEach(p => { if (p.id !== panelId) p.classList.remove('show'); });
...
window.onclick = function (event) {
    ...
    if (!event.target.closest('.loc-selector-box') && !event.target.closest('#loc-dropdown')) {
...
function openModal() { document.getElementById('address-modal').style.display = 'flex'; }
function closeModal() { document.getElementById('address-modal').style.display = 'none'; }
function saveAddress() {
    ...
    if (document.getElementById('loc-selected-text').innerText === 'Chọn' || !selectedLocation.w) {
        alert("Vui lòng chọn đầy đủ cấp hành chính (Tỉnh/Thành Phố, Quận/Huyện, Phường/Xã)!"); return;
    }
    if (!detail) {
        alert("Vui lòng nhập Địa chỉ chi tiết!"); return;
    }
...
function completeRegistration(event) {
    ...
    document.querySelector('.header-title').innerText = "Kênh Người Bán - Bảng Điều Khiển";
...
function toggleLocationDropdown(e) {
    ...
    if (!selectedLocation.p) renderLocList(addressData.map(p => p.name), 'p');
}
...
function clickLocTab(type) {
    ...
    if (type === 'p') {
        ...
        renderLocList(addressData.map(p => p.name), 'p');
    } else if (type === 'd') {
        if (!selectedLocation.p) return alert('Vui lòng chọn Tỉnh/Thành phố trước!');
        ...
        switchLocTab('ward');
        const pData = addressData.find(p => p.name === selectedLocation.p);
        const dData = pData ? pData.districts.find(d => d.name === selectedLocation.d) : null;
    ...
    dataArray.forEach(itemStr => {
        ...
        div.onclick = function () {
            if (type === 'p') {
                ...
                document.getElementById('loc-selected-text').innerText = `${selectedLocation.p}, ${selectedLocation.d}, ${selectedLocation.w}`;
...
function generateRandomOrder() {
    ...
    if (orderCount === 0 && document.querySelectorAll('.order-item').length === 0) {
        container.innerHTML = '';
    }

    let randomProd = productTypes[Math.floor(Math.random() * productTypes.length)];
...
function processOrder(btn, type, price) {
    ...
    if (container.children.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:#999; padding:20px;">Chưa có đơn hàng nào chờ xử lý</div>';
    }
    ...
    if (revTextEl) revTextEl.innerText = new Intl.NumberFormat('vi-VN').format(totalRevAmount);
